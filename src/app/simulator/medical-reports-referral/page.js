...
export default function Home() {
  ...
  const onImageRetrieve = async payload => {
    ...
    try {
      const response = await fetch("/api/simulator/medical-reports-referral", {
        method: "POST",
      ...
      if (response.ok) {
        ...
        setResponse({ status: response.status, message: 'Medical reports', data: { ...data.data }, rawData: data })
      } else {
